	...
	this.frame_1 = function() {
		...
		*/
		...
		var objName = "";
			...
			*/
			...
			console.log(stage.sortChildren());
			...
			console.log("this.containerHeddin.parent---->" + this.containerHeddin.parent);		
			console.log("this.MC_chip10.parent---->" + that["MC_chip10"].parent);		
			console.log("container[01].parent---->" + container[01].parent);		
		...
		*/
		...
		this.nextAttack = function () {
			...
			//カード表示のセット
			right = right+2;
			container[right] = trumpArrya[right];
			console.log("container[right]-----------" + container[right]);
		
			left = left + 2;
			container[left] = trumpArrya[left];
			console.log("container[left]------------" + container[left]);
		
			container[right] = images[container[right]];
			container[left] = images[container[left]];
			...
			//割った数のあまりがカードの番号
			//あまりが0の時はカード番号はK
			visibleCardMunber = (container[right] + 1) % 13;
			if (visibleCardMunber == 0) {
				visibleCardMunber = 13;
			}
		
			hiddenCardMunber = (container[left] + 1) % 13;
			if (hiddenCardMunber == 0) {
				hiddenCardMunber = 13;
			}
		
			visibleCardMark = (container[right] + 1 - visibleCardMunber) / 13;
			hiddenCardMark = (container[left] + 1 - hiddenCardMunber) / 13;
